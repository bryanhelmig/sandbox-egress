...
if [ "$#" -gt 9 ]; then
  echo "usage: $0 [lease-runs] [lease-batches] [idle] [tls] [terminal-runs] [terminal-batches] [headers] [failed-start-runs] [failed-start-batches]" >&2
  exit 2
fi
...
failed_start_runs_per_batch=${8:-250}
failed_start_batches=${9:-4}
