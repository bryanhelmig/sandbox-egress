// External consumer for the generic Linux host-network conformance lane.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error = old
        .close(Instant::now())
        .expect_err("expired close retains the owner");
    assert_eq!(
        error.kind(),
        sandbox_egress::CloseErrorKind::DeadlineExceeded
    );
    assert!(matches!(
        proxy.attach(identity.clone(), policy(second_address.port())?),
        Err(sandbox_egress::AttachError::IdentityInUse)
    ));
    println!("RETRY ownership=retained identity=rejected");
    let old_usage = error
        .into_lease()
